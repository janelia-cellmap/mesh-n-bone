...
`box_size` can be a scalar (applied to all axes) or a 3-element list for per-axis control.
...
| Option | Description |
|--------|-------------|
| `INPUT_FILE` | Path to input mesh (e.g. `.ply`, `.obj`) |
| `OUTPUT_FILE` | Path to write skeleton output |
| `--subdivisions N` | Number of loop subdivision iterations (default: 1) |
| `--neuroglancer` | Input mesh is in neuroglancer format |
